...
class CefBrowserBackend : BrowserBackend, EventListener {
    ...
    override fun makeDependenciesAvailable(taskManager: TaskManager, whenAvailable: () -> Unit) {
        ...
        if (!MCEF.INSTANCE.isInitialized) {
            ...
            // On Android/mobile launchers this returns false; skip gracefully instead of crashing.
            if (!resourceManager.isSystemCompatible) {
                logger.warn("JCEF is not compatible with this system (likely a mobile launcher). " +
                    "Browser-dependent features will be disabled.")
            ...
            if (resourceManager.requiresDownload()) {
                taskManager.launch("MCEF") { task ->
                    ...
                    runCatching {
                        ...
                        // On mobile/Android the download either hangs or fails because
                        // CEF binaries are x86-only. Fall back to no-browser mode.
                        logger.error("Failed to download JCEF, disabling browser features.", ex)
                        BrowserBackendManager.isSkipping = true
                        // Don't crash — just leave the task to complete naturally
                    }
                }
            } else {
                whenAvailable()
